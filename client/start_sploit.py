...
HEADER = r'''
  _______ ____ _______ _______ _____ 
...
def post_flags(args, flags):
    ...
    data = [{'flag': item['flag'], 'exploit_name': sploit_name, 'team_ip': item['team']}
            for item in flags]
...
class FlagStorage:
    ...
    def add(self, flags, team_addr):
        with self._lock:
            for item in flags:
                if item not in self._flags_seen:
                    self._flags_seen.add(item)
                    self._queue.append({'flag': item, 'team': team_addr})
...
def process_sploit_output(stream, args, team_addr, flag_format, attack_no):
    try:
        output_lines = []
        instance_flags = set()
        ...
        while True:
            ...
            if line_flags:
                flag_storage.add(line_flags, team_addr)
        ...
        if attack_no <= args.verbose_attacks and not exit_event.is_set():
            ...
            display_sploit_output(team_addr, output_lines)
            if instance_flags:
                logging.info('Got {} flags from "{}": {}'.format(
                    len(instance_flags), team_addr, instance_flags))
    except Exception as e:
...
def launch_sploit(args, team_name, team_addr, attack_no, flag_format):
    ...
    threading.Thread(target=lambda: process_sploit_output(
        proc.stdout, args, team_addr, flag_format, attack_no)).start()
...
def show_time_limit_info(args, config, max_runtime, attack_no):
    if attack_no == 1:
        min_attack_period = config['flag_lifetime'] - config['round_duration'] - POST_PERIOD
